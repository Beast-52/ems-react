...
const EmployeeTable = () => {
  ...
  const getTaskOverview = () => {
    ...
    const taskSummary = usersData.map((user) => {
      const userTasks = taskData.find((item) => item.userId === user.userId)?.tasks || []; // Use optional chaining
      const summary = {
        name: user.name,
    ...
    <div className="mt-5 w-full h-[20vh] relative">
      <table className="text-white border-separate relative mx-10 w-[96%] border-spacing-y-4 text-3xl">
        <thead className="bg-gray-700 sticky top-0 left-0">
          <tr>
            <th className="border-b border-gray-700 py-3 px-4 text-left">Employee Name</th>
            <th className="border-b border-gray-700 py-3 px-4 text-left">Total</th>
            <th className="border-b border-gray-700 py-3 px-4 text-left">New Task</th>
            <th className="border-b border-gray-700 py-3 px-4 text-left">Active Task</th>
            <th className="border-b border-gray-700 py-3 px-4 text-left">Completed</th>
            <th className="border-b border-gray-700 py-3 px-4 text-left">Failed</th>
          </tr>
        </thead>
        <tbody>
          {taskOverview.map((task, index) => (
            <tr key={index} className="outline rounded outline-gray-700 text-2xl">
              <td className="py-3 px-4">{task.name}</td>
              <td className="py-3 px-10">
                <a href="#" className="text-[#4682B4]">{task.totalCount}</a>
              </td>
              <td className="py-3 px-20 text-[#ff5722]">{task.newTasksCount}</td>
              <td className="py-3 px-20 text-blue-500">{task.activeCount}</td>
              <td className="py-3 px-20 text-green-500">{task.completedCount}</td>
